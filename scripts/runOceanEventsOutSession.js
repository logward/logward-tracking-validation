#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
//  scripts/runOceanEventsOutSession.js
//
//  Interactive E2E Events-Out test session for Logward ↔ Shippeo ocean tracking.
//
//  Usage:
//    node scripts/runOceanEventsOutSession.js
//
...
const { getShippeoToken }  = require('../helpers/shared/shippeoAuth');
...
function spawnPlaywright(grepPattern) {
  return new Promise(resolve => {
    const args = ['playwright', 'test', '--project=ocean', 'OceanEventsOut', '--grep', grepPattern, '--reporter=list'];
